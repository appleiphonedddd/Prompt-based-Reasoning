...
BASELINE_REGISTRY: dict[str, tuple] = {
    ...
    "fot": (FoT, lambda a: dict(
        ...
        orbit_majority=a.fot_orbit_majority,
...
class Evaluator:
    ...
    def build_baseline(self, client, dataset=None):
        ...
        if self.args.baseline.lower() == "fot":
            ...
            # relations to transform the query with (metamorphic regime).
...
def fot_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument("--fot_budget", type=int, default=3,
                   help="Budget K: maximum number of Falsify→Repair iterations before "
                        "the best measured candidate is returned")
    g.add_argument("--fot_probes", type=int, default=3,
                   help="Number of probes n: relations drawn from the catalogue C per "
                        "falsification attempt, i.e. the size of the orbit. Each costs "
                        "one Solve call (plus at most one to build the variant), so "
                        "total cost is K·(1+n). The executable regime ignores it: one "
                        "probe settles a deterministic checker's verdict.")
    g.add_argument("--fot_tau", type=int, default=2,
                   help="Refutation threshold τ: relations that must be violated before "
                        "a repair fires (Remark 3). Repair additionally requires the "
                        "candidate to be outside the majority of its orbit.")
    g.add_argument("--fot_relations", nargs="+", default=None,
                   help="Restrict the relation catalogue C to these relation names "
    ...
    g.add_argument("--no_fot_orbit_majority", action="store_false", dest="fot_orbit_majority",
                   help="Ablation: drop the orbit-majority acceptance rule, so a "
                        "violation count of τ alone triggers a repair. With --fot_tau 1 "
                        "this recovers the pilot's single-violation trigger.")
    g.add_argument("--fot_solve_temp", type=float, default=0.0,
                   help="Sampling temperature for Solve — the initial one and every "
                        "follow-up solve of a variant. Independence comes from "
                        "perturbing the input, not from resampling, so 0 is fine.")
